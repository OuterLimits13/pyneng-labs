...
print('\nTrunks:')
#solution. Trunk if's
for intf, allowed_vlans in fast_int['trunk'].items():
	print('interface FastEthernet' + intf)
	for command in trunk_template:
		if command.endswith('allowed vlan') and allowed_vlans[0].endswith('add'):
			print(' {} add {}'.format(command, ','.join(allowed_vlans).lstrip('add,')))
		elif command.endswith('allowed vlan') and allowed_vlans[0].endswith('only'): 
			print(' {} {}'.format(command, ','.join(allowed_vlans).lstrip('only,') ) )   
		elif command.endswith('allowed vlan') and allowed_vlans[0].endswith('del'): 
			print(' {} remove {}'.format(command, ','.join(allowed_vlans).lstrip('del,') ) )   
		else:
			print(' {}'.format(command))
